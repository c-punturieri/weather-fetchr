...
Complete as of 06/2025.

## Notes

This script pulls weather data using the ACIS RCC query builder.

Note that these data are based off of participants who have good GPS data (N = 146).
...
Next step is to calculate lat/lon where individual spent the most time on a given day. If multiple maximum duration points, pull first from list.
```{r}
...
gps_longest |> 
  select(subid, lat_rounded, lon_rounded) |>
  distinct(subid, .keep_all = TRUE) |> 
  head()
```
...
We are going to pull seven days of weather data prior to study start from an individual's home address.
```{r}
gps_home <- gps |> filter((type == "home" | type == "temporaryresidence") & known_loc == TRUE) |> group_by(subid) |> slice(1) |> select(subid, lat_rounded, lon_rounded)
```

Add in seven days prior to study start to gps_longest.
```{r}
gps_prior <- gps_longest |>
  group_by(subid) |> 
  slice(1) |>
  mutate(date_seq = list(seq(date - 7, date - 1, by = "day"))) |> 
  unnest(date_seq) |> 
  select(subid, date = date_seq) |> 
  ungroup()
```

Merge prior seven days with home/temporary residence location.
```{r}
gps_prior <- gps_prior |> 
  left_join(gps_home, by = "subid")
```

Add in prior study days to longest gps point data set. Kind of kludge but we will remake the day_label variable here, too, since we are only using it as a looping marker.
```{r}
gps_longest <- rbind(gps_longest, gps_prior) |>
  arrange(subid, date) |>
  group_by(subid, date) |> 
  mutate(day_label = cur_group_id()) |> 
  ungroup()
...
Retrieve database of existing weather stations. Eventually want to pull data up to a week prior and want to make sure last day of study is included, so adding an 8 and 1 day buffer on either side of participation.
```{r}
...
weather_stns <- rbind(FluMoDL::NOAA_countryStations(fips = "US",
                                                    from = first_date - 8, to = last_date + 1),
...
Set all times to 11:59:59 PM America/Chicago. This is important because we want to make sure that weather data are not used on the *same day* they are collected.
```{r}
weather <- weather |> 
  mutate(dttm_obs = make_datetime(
    year = year(date),
    month = month(date),
    day = day(date),
    hour = 23,
...
```{r}
weather |>
  write_csv(here::here(path_shared, "weather.csv"))
```